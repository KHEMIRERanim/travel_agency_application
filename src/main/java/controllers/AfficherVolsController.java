...
public class AfficherVolsController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // S'assurer que seul mainPane est visible au démarrage
        if (mainPane != null) mainPane.setVisible(true);
        if (editPane != null) editPane.setVisible(false);
    ...
    private VBox createFlightCard(Flight flight) {
        ...
        // Action pour modifier un vol - MODIFIÉ POUR AFFICHER DANS LA MÊME PAGE
        modifyButton.setOnAction(event -> {
            try {
                showModifyFlightForm(flight);
            } catch (IOException e) {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Erreur lors de l'affichage du formulaire de modification: " + e.getMessage());
            }
        });
    ...
    // Nouvelle méthode pour afficher le formulaire de modification dans la même page
    private void showModifyFlightForm(Flight flight) throws IOException {
        // Cacher la liste des vols
        mainPane.setVisible(false);

        // Charger le formulaire de modification
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/ModifierVol.fxml"));
        Parent modifyFormRoot = loader.load();

        // Récupérer le contrôleur et initialiser avec les données du vol
        ModifierVolController controller = loader.getController();
        controller.initData(flight);

        // Configurer un bouton de retour
        controller.setOnCloseCallback(() -> {
            editPane.getChildren().clear();
            mainPane.setVisible(true);
            editPane.setVisible(false);
            // Recharger les vols pour refléter les modifications
            loadFlights();
        });

        // Afficher le formulaire de modification
        editPane.getChildren().clear();
        editPane.getChildren().add(modifyFormRoot);
